...
{
    public partial class frmBseSetEdit : frmEditorBase
    {
        #region 变量
        private DataSet dsLoad = null;
        private DataTable dtSet = null;
        ...
        public override void RefreshItem()
        {
            ...
            blInitBound = true;
            this.gcSet.DataSource = dtSet.DefaultView;//可能引发gridView1_FocusedRowChanged
            gvSet.BestFitColumns();
            blInitBound = false;

            gvSet_FocusedRowChanged(null, new DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs(-1, gvSet.FocusedRowHandle));
        }
        ...
        private void gvSet_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            if (e.FocusedRowHandle <= -1 || blInitBound)
                return;

            DataRow dr = gvSet.GetFocusedDataRow();
            if (dr == null)
                return;

            blInitBound = true;
            frmDataTable.DefaultView.RowFilter = "SetKey='" + dr["SetValue"].ToString() + "'";
        ...
        public override void SetWMode(string strMode)
        {
            ...
            switch (strMode)
            {
                ...
                case "ADD":
                    DataRow drSet = gvSet.GetFocusedDataRow();
                    if (drSet == null)
                        return;
        ...
        private void DoSave()
        {
            ...
            if (Convert.ToString(dr["Fy_Id"])==string.Empty 
                || dr["Fy_Id"].ToString() == "-9999")
            {
                dr["Fy_Id"] = "-1";
            }
            if (dr["DeptNme"].ToString() == string.Empty)
            {
                dr["Dept_Id"] = "-1";
            }
            if (dr["UserNme"].ToString() == string.Empty)
            {
                dr["User_Id"] = "-1";
            }
            dr.EndEdit();
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            if (k == 32)
            {
                if (strFocusedContrName == "txtDeptNme")
                {
                    if (frmEditorMode == "VIEW")
                        return true;
                    DataRow drInfo = gridVMain.GetFocusedDataRow();
                    if (drInfo == null)
                        return true;
                    ...
                    {
                        DataRow dr = frm.SelectRow;
                        string[] strFieldVs = "Dept_Id".Split(",".ToCharArray());
                        foreach (string strFieldV in strFieldVs)
                        {
                            drInfo[strFieldV] = dr[strFieldV];
                        }
                        drInfo["DeptNme"] = dr["Name"];
                        drInfo.EndEdit();
                    }
                    return true;
                }
                else if (strFocusedContrName == "txtUserNme")
                {
                    if (frmEditorMode == "VIEW")
                        return true;
                    DataRow drInfo = gridVMain.GetFocusedDataRow();
                    ...
                    {
                        DataRow dr = frm.SelectRow;
                        string[] strFieldVs = "User_Id".Split(",".ToCharArray());
                        foreach (string strFieldV in strFieldVs)
                        {
                            drInfo[strFieldV] = dr[strFieldV];
                        }
                        drInfo["UserNme"] = dr["Name"];
                        drInfo.EndEdit();
                    }
                    return true;
                }
