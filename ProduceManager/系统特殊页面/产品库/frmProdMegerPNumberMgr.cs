...
{
    public partial class frmProdMegerPNumberMgr : frmEditorBase
    {
        public frmProdMegerPNumberMgr()
        {
            InitializeComponent();
        }
        ...
        //窗体载入
        private void frmProdMegerPNumberMgr_Load(object sender, EventArgs e)
        {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    if (DialogResult.No == MessageBox.Show("您的图片区还有挑选的款未进行合并，是否合并？", "提示", MessageBoxButtons.YesNo))
            //    {
            //        this.grpPic.Controls.Clear();
            //        x = 10;
            //        y = 25;
            //        this.lupMegerPNumber.Properties.DataSource = null;
        ...
        private DataTable GetTable()
        {
            ...
            string[] strKey = "EUser_Id,EDept_Id,Fy_Id,Company_Id,Name,PNumber,PNumber,Designer,PatentNumber,MarketTime,Help_Factory,Pc_Id,Material,Technology,Sex,DesignType,Material_Pc_Id,Compound_Type,SizeSmall,SizeLarger,WeightSmall,WeightLarger,Flag".Split(",".ToCharArray());//BasicTechnology,BasicTechnologySurface,BseShapeFlower,BseShapeSquare,BseRound,BseFaceWidth,BseFastener,BseStructure,Flag
            string[] strVal = new string[] {
                     CApplication.App.CurrentSession.UserId.ToString(),
                     CApplication.App.CurrentSession.DeptId.ToString(),
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        //getImageByte(dt.Rows[i]["StylePic"].ToString());
                        //dr["PhotoName"] = StaticFunctions.ImgToByt(Image.FromFile(ImagePath));
                    }
        ...
        {
            if (this.grpPic.Controls.Count <= 3)
            {
                MessageBox.Show("只选择一个营销款号不能进行合并！");
                return;
            }

            if (lupMegerPNumber.Text.Trim().Length == 0)
            {
                MessageBox.Show("请选择合并款号！");
                return;
            }
            ...
            string[] strKey = "EUser_Id,EDept_Id,Fy_Id,Company_Id,MergePNumber,Key_Ids,Flag".Split(",".ToCharArray());
            string[] strVal = new string[] {
                     CApplication.App.CurrentSession.UserId.ToString(),
                     CApplication.App.CurrentSession.DeptId.ToString(),
                     CApplication.App.CurrentSession.FyId.ToString(), 
                           CApplication.App.CurrentSession.FyId.ToString(), 
                     lupMegerPNumber.Text.Trim(),
                  PmIds,
                     "101" };
        ...
        private void lblGoTo_Click(object sender, EventArgs e)
        {
            ...
            frmProdMegerPNumberMgrEdit Log = new frmProdMegerPNumberMgrEdit(dsLog);
            Log.Show();
        }
