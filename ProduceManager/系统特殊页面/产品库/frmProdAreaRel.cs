...
{
    public partial class frmProdAreaRel : frmEditorBase
    {
        string strSpName = "Bse_Prod_Area_Add_Edit_Del";
        string strKeyId = "-1";
        DataSet dsLoad = null;
        private string CustStr = string.Empty;
        public frmProdAreaRel()
        {
            InitializeComponent();
        }

        private void frmProdAreaRel_Load(object sender, EventArgs e)
        {
            ...
            StaticFunctions.BindCheckedComboBoxEdit(chkComArea, dsLoad.Tables[0], "Area_Name", "Area_Id", "", "");
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            string[] strAry = this.chkComArea.Text.Split(',');
        ...
        private void gridView1_EndSorting(object sender, EventArgs e)
        {
            ...
            if (visibleIndex != "0" && visibleIndex != "1")
            {
                ...
                int x = a.X;
                int y = a.Y;
                frmSelectLimitsArea frmlimits = new frmSelectLimitsArea(true, columnname, x, y);
            ...
            for (int j = 3; j < this.gridView1.Columns.Count; j++)
            {
                for (int i = 0; i < gridView1.RowCount; i++)
                {
                    if (i == gridView1.RowCount - 1)
                    {
                        ...
                        first = first + "SELECT '" + gridView1.GetDataRow(i)[0].ToString() + "', '" + gridView1.GetDataRow(i)[j].ToString() + "','" + this.gridView1.Columns[j].FieldName +  "'";
                    ...
                    else
                    {
                        ...
                        { 
                            first = first + "SELECT '" + gridView1.GetDataRow(i)[0].ToString() + "', '" + gridView1.GetDataRow(i)[j].ToString() + "','" + this.gridView1.Columns[j].FieldName + "' UNION ALL ";
                        }
                    }
                }
            }
        ...
        {
            string[] strKey = "Key_Id,EUser_Id,EDept_Id,Fy_Id,Bus_PNumber,flag".Split(",".ToCharArray());
            string[] strVal = new string[] {strKeyId,
                     CApplication.App.CurrentSession.UserId.ToString(),
                     CApplication.App.CurrentSession.DeptId.ToString(),
                     CApplication.App.CurrentSession.FyId.ToString(),
                     ...
                     "5" };
            DataTable dt = this.DataRequest_By_DataTable(strSpName, strKey, strVal);
            labelControl6.Text = dt.Rows[0]["Bus_PNumber"].ToString();
